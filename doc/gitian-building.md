...
*Setup instructions for a gitian build of FreedomCoin Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the FreedomCoin
...
to the FreedomCoin GitHub release page.
...
- [Building FreedomCoin Core](#building-freedomcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/freedomcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/freedomcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/freedomcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/freedomcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/freedomcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/freedomcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/freedomcoin/gitian.sigs/) repository:
...
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/freedomcoin-osx-*-build.assert
```
