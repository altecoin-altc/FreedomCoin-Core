FreedomCoin Core version *4.2.0* is now available from:  <https://github.com/freedomcoin-project/freedomcoin/releases>
...
be used in v4.2 by starting freedomcoind with the '-deprecatedrpc=accounts'
...
- The "connections" button in the GUI topbar can be clicked to open the network monitor dialog ([#1688](https://github.com/FreedomCoin-Project/FreedomCoin/pull/1688)).
...
If the stake split is active (threshold > 0), then stake split threshold value must be greater than a minimum, set by default at 100 FREED. The minimum value can be changed using the `-minstakesplit` startup flag ([#1586](https://github.com/FreedomCoin-Project/FreedomCoin/pull/1586)). A value `0` is still allowed, regardless of the minimum set, and, as before, can be used to disable the stake splitting functionality.

### Changed command-line options

- new command `-minstakesplit` to modify the minimum allowed for  the stake split threshold ([#1586](https://github.com/FreedomCoin-Project/FreedomCoin/pull/1586)).

- new commands `-maxmempool`, to customize  the memory pool size limit, and `-checkmempool=N`, to customize the frequency of the mempool check ([#1647](https://github.com/FreedomCoin-Project/FreedomCoin/pull/1647)).

- new commands `-limitancestorcount=N` and `limitancestorsize=N`, to limit the number and total size of all in-mempool ancestors for a transaction ([#1647](https://github.com/FreedomCoin-Project/FreedomCoin/pull/1647)).

- new commands `-limitdescendantcount=N` and `limitdescendantsize=N`, to limit the number and total size of all in-mempool descendants for a transaction ([#1647](https://github.com/FreedomCoin-Project/FreedomCoin/pull/1647)).
...
- The `asm` property of each scriptSig now contains the decoded signature hash type for each signature that provides a valid defined hash type ([#1633](https://github.com/FreedomCoin-Project/FreedomCoin/pull/1633)).<br>
The following items contain assembly representations of scriptSig signatures
and are affected by this change: RPC `getrawtransaction`, RPC `decoderawtransaction`, REST `/rest/tx/` (JSON format), REST `/rest/block/` (JSON format when including extended tx details), `freedomcoin-tx -json`

### Modified input/output for existing commands

- new "usage" field in the output of `getmempoolinfo`, displaying the total memory usage for the mempool ([#1645](https://github.com/FreedomCoin-Project/FreedomCoin/pull/1645)).

- new "upgrades" field in the output of `getblockchaininfo`, showing upcoming and active network upgrades ([#1665](https://github.com/FreedomCoin-Project/FreedomCoin/pull/1665), [#1687](https://github.com/FreedomCoin-Project/FreedomCoin/pull/1687)).
...
- `patriotnodedebug`. Use `getpatriotnodestatus` instead. ([#1698](https://github.com/FreedomCoin-Project/FreedomCoin/pull/1698)).
...
### Core Features
 ...
 - #1729 `76ea490ac1` [Core] Add checkpoints for FreedomCoin v4.1.1 enforcement (random-zebra)
...
### Build System
 - #1681 `131ec069fd` [Build] Set complete cpp/cxx flags for bench_freedomcoin binary (Fuzzbawls)
...
### GUI
 ...
 - #1548 `176d3ae558` [Cleanup][GUI] Remove zFREED faqs (random-zebra)
 - #1549 `47bf23aa14` [Cleanup][GUI] Nuke zFREED from the GUI (random-zebra)
...
### RPC/REST
 ...
 - #1660 `10876c6c80` [RPC] Change btc to FREED in help text (PeterL73)
