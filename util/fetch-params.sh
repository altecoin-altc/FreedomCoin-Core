...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/FreedomCoinParams"
    else
        PARAMS_DIR="$HOME/.freedomcoin-params"
    fi
fi
...
Failed to fetch the FreedomCoin zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
FreedomCoin - fetch-params.sh

This script will fetch the FreedomCoin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common FreedomCoin zkSNARK parameters. Note that it is
