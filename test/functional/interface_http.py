...
class HTTPBasicsTest (FreedomCoinTestFramework):
    ...
    def run_test(self):
        ...
        out1 = conn.getresponse().read()
        assert(b'"error":null' in out1)
        assert(conn.sock is not None) #connection must be closed because freedomcoind should use keep-alive by default
