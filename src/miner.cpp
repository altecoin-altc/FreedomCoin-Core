...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("FreedomCoinMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("FreedomCoinMiner : ProcessNewBlock, block not accepted");
    }
...
void CheckForCoins(CWallet* pwallet, std::vector<CStakeableOutput>* availableCoins)
{
    ...
    fMasternodeSync = sporkManager.IsSporkActive(SPORK_22_STAKE_BEFORE_MNSYNC) || !patriotnodeSync.NotCompleted();
}

void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    LogPrintf("FreedomCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("freedomcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running FreedomCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("FreedomCoinMiner exception");
    } catch (...) {
        LogPrintf("FreedomCoinMiner exception");
    }

    LogPrintf("FreedomCoinMiner exiting\n");
}
