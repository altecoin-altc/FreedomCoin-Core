...
static const std::map<ProRegParam, std::string> mapParamHelp = {
        {collateralAddress,
            "%d. \"collateralAddress\"     (string, required) The FreedomCoin address to send the collateral to.\n"
        },
        {collateralHash,
        ...
        {ownerAddress,
            "%d. \"ownerAddress\"          (string, required) The FreedomCoin address to use for payee updates and proposal voting.\n"
        ...
        {payoutAddress_register,
            "%d. \"payoutAddress\"          (string, required) The FreedomCoin address to use for patriotnode reward payments.\n"
        },
        {payoutAddress_update,
            "%d. \"payoutAddress\"          (string, required) The FreedomCoin address to use for patriotnode reward payments.\n"
...
// Allows to specify FreedomCoin address or priv key (as strings). In case of FreedomCoin address, the priv key is taken from the wallet
...
static CKeyID ParsePubKeyIDFromAddress(const std::string& strAddress)
{
    ...
    if (!keyID) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprintf("invalid FreedomCoin address %s", strAddress));
    }
    return *keyID;
}
...
void RegisterEvoRPCCommands(CRPCTable &tableRPC)
{
    if (!Params().IsRegTestNet()) {
        // Disabled before FreedomCoin v6.0
        return;
    }
