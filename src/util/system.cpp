...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "freedomcoind -foo=bar
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FreedomCoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FreedomCoinParams
    // Mac: ~/Library/Application Support/FreedomCoinParams
    // Unix: ~/.freedomcoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FreedomCoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "FreedomCoinParams";
#else
    // Unix
    return pathRet / ".freedomcoin-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/freedomcoin/sapling-spend.params";
        sapling_output = "/usr/share/freedomcoin/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/freedomcoin/sapling-spend.params";
            sapling_output = "/usr/local/share/freedomcoin/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", FreedomCoin_PATRIOTNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()) {
        FILE* configFile = fsbridge::fopen(pathConfigFile, "a");
         if (configFile != NULL) {
            std::string strHeader = "# FreedomCoin config file\n";
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", FreedomCoin_PID_FILENAME));
