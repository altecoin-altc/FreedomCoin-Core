...
#include "qt/freedomcoin/forms/ui_patriotnodeswidget.h"

#include "qt/freedomcoin/qtutils.h"
#include "qt/freedomcoin/mnrow.h"
#include "qt/freedomcoin/mninfodialog.h"
#include "qt/freedomcoin/patriotnodewizarddialog.h"
...
PatriotNodesWidget::PatriotNodesWidget(FreedomCoinGUI *parent) :
    PWidget(parent),
...
void PatriotNodesWidget::onCreatePNClicked()
{
    ...
    if (walletModel->getBalance() <= mnCollateralAmount) {
        inform(tr("Not enough balance to create a patriotnode, %1 required.")
            .arg(GUIUtil::formatBalance(mnCollateralAmount, BitcoinUnits::FREED)));
        return;
    }
    showHideOp(true);
