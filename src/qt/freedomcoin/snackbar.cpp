...
#include "qt/freedomcoin/forms/ui_snackbar.h"
#include "qt/freedomcoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(FreedomCoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &FreedomCoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
