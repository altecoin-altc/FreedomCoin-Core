...
#include "qt/freedomcoin/settings/settingsfaqwidget.h"
#include "qt/freedomcoin/settings/forms/ui_settingsfaqwidget.h"
...
SettingsFaqWidget::SettingsFaqWidget(FreedomCoinGUI* parent, ClientModel* _model) :
    QDialog(parent),
    ...
    clientModel(_model)
{
    ...
    setCssProperty({
           ...
           ui->labelNumber_UnspendableFREED,
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableFREED,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableFREED,
    ...
    setCssProperty({
              ui->pushButton_Intro,
              ui->pushButton_UnspendableFREED,
              ui->pushButton_Stake,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("FreedomCoin is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. FreedomCoin incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("FreedomCoin utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of FreedomCoin to participate in earning block rewards "
               "while securing the network with full node wallets, as well as to "
               "run Patriotnodes to create and vote on proposals.")));
    ui->labelContent_Intro->setText(introContent);

    QString unspendableFREEDContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received FreedomCoin requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your FreedomCoin wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableFREED->setText(unspendableFREEDContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of FreedomCoin with a minimum of 480 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("freedomcoin-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString patriotnodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A patriotnode is a computer running a full node %1 wallet with a "
               ...
               "regularly. These services include:")
                .arg(PACKAGE_NAME)
                .arg(GUIUtil::formatBalance(clientModel->getPNCollateralRequiredAmount(), BitcoinUnits::FREED)) +
        ...
        formatFAQParagraph(
            ...
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in FreedomCoin Governance")) +
                formatFAQListItem(tr("Earn Patriotnode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the FreedomCoin network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendableFREED, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableFREED);});
    ...
    if (parent)
        connect(parent, &FreedomCoinGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
