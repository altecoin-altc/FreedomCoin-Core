...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select FREED Outputs to Spend" :
            "Select Shielded FREED to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::FREED;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show FreedomCoin address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
