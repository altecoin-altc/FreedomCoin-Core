...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::FREED);
    ...
    if (settings.contains("nAnonymizeFreedomCoinAmount") || reset)
        gArgs.SoftSetArg("-anonymizefreedomcoinamount", settings.value("nAnonymizeFreedomCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in freedomcoin.cpp during shotdown to also remove the window positions
