...
#include "config/freedomcoin-config.h"
#endif
...
#include "qt/freedomcoin/splash.h"
#include "qt/freedomcoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("freedomcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in freedomcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in freedomcoin.qrc)
...
/** Class encapsulating FreedomCoin Core startup and shutdown.
...
/** Main FreedomCoin application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (FreedomCoinGUI)
...
{
    window = new FreedomCoinGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &FreedomCoinGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &FreedomCoinGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(FreedomCoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(FreedomCoinGUI::DEFAULT_WALLET);
        }
#endif
        ...
        // FreedomCoin: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &FreedomCoinGUI::handlePaymentRequest);
        connect(window, &FreedomCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. FreedomCoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(freedomcoin_locale);
    Q_INIT_RESOURCE(freedomcoin);
    ...
    /// 6. Determine availability of data and blocks directory and parse freedomcoin.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", FreedomCoin_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // freedomcoin: links repeatedly have their payment requests routed to this process:
