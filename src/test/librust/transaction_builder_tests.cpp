...
BOOST_AUTO_TEST_CASE(TransparentToSapling)
{
    ...
    // 0.5 t-FREED in, 0.4 shielded-FREED out, 0.1 t-FREED fee
...
BOOST_AUTO_TEST_CASE(SaplingToSapling)
{
    ...
    // --- 0.4 shielded-FREED in, 0.299 shielded-FREED out, 0.1 shielded-FREED fee, 0.001 shielded-FREED change (added to fee)
    ...
    // --- Now try with 1 shielded-FREED in, 0.5 shielded-FREED out, 0.1 shielded-FREED fee, 0.4 shielded-FREED change
...
BOOST_AUTO_TEST_CASE(FailsWithNegativeChange)
{
    ...
    // Generate a 0.5 FREED note
    auto testNote = GetTestSaplingNote(pa, 59990000);

    // Fail if there is only a Sapling output
    // 0.5 shielded-FREED out, 0.1 t-FREED fee
    ...
    // 0.5 t-FREED out, 0.1 t-FREED fee
    ...
    // 0.5 t-FREED out, 0.1 t-FREED fee, 0.59999 shielded-FREED in
    builder.AddSaplingSpend(expsk, testNote.note, testNote.tree.root(), testNote.tree.witness());
    BOOST_CHECK_EQUAL("Change cannot be negative", builder.Build().GetError());
