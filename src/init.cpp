...
// Copyright (c) 2015-2021 The FreedomCoin developers
...
void PrepareShutdown()
{
    ...
    util::ThreadRename("freedomcoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf("Specify configuration file (default: %s)", FreedomCoin_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", FreedomCoin_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf("Disable all FreedomCoin specific functionality (Patriotnodes, Budgeting) (0-1, default: %u)", 0));
    ...
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf("Specify patriotnode configuration file (default: %s)", FreedomCoin_PATRIOTNODE_CONF_FILENAME));
...
{
    util::ThreadRename("freedomcoin-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that FreedomCoin is running in a usable environment with all
...
#endif
    LogPrintf("FreedomCoin version %s\n", version_string);
}
...
{
    // Make sure only a single FreedomCoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", FreedomCoin_CONF_FILENAME)).string());
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //FreedomCoin specific: zerocoin and spork DB's
                ...
                // FreedomCoin: load previous sessions sporks if we have them.
